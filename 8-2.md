...
這 3 個 method 都叫做 public services or public interface
...
```
會 call 這一個 ```IllegalArgumentException``` method
...
```
這樣就會呼叫那一個 class 的 constructor

In this case, we call the constructor that allows
us to specify a custom error message. 
...
universal-time format, consisting of two digits each for the hour, minute and second—recall that you can use the 0 flag in a printf format specification (e.g., ```"%02d"```) to display
...
Line 30 uses ```static``` method format of class String to return a String containing the formatted hour, minute and second values, each with two digits and possibly a leading 0
(specified with the 0 flag). 
...
1. format() 跟 System.out.printf 很像, 不同的地方 format() 是 return String, System.out.printf 是顯示在小黑視窗
...
by AM or PM (e.g., 11:30:17 AM or 1:27:06 PM). 
...
- 在 code 裡面的 ```toString``` 跟 ```System.out.printf``` 很像, 只是方便用來 format string 的東西而已
